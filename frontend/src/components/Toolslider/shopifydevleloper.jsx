...
import { FaShopify, FaGithub, FaGoogle } from "react-icons/fa";
import { SiReact, SiVite, SiTailwindcss, SiGraphql, SiPostman, SiShopify, SiFirebase } from "react-icons/si";

// Shopify Developer Tools List
const shopifyTools = [
  { name: "Shopify", icon: <FaShopify size={30} className="text-green-" /> },
  { name: "Shopify CLI", icon: <SiShopify size={30} className="text-green" /> },
  { name: "React.js", icon: <SiReact size={30} className="text-blue" /> },
  ...
  return (
    <div className="bg-gray-1 py-10 mt-11 mb-10 px-4 md:px-6 lg:px-10">
      <h2 className="text-3xl font-bold text-center text-gray mb-6">
        Shopify Development Tools You Will Master
      </h2>
      ...
      <Swiper
        ...
        className="w-full"
      >
        {Array.from({ length: Math.ceil(shopifyTools.length / 2) }, (_, rowIndex) => (
          <SwiperSlide key={rowIndex}>
            <div className="grid grid-cols-1 gap-y-6 mb-14 items-center justify-center">
              {shopifyTools.slice(rowIndex * 2, rowIndex * 2 + 2).map((tool, index) => (
                <div
                  key={index}
...
export default ShopifyToolsSlider;