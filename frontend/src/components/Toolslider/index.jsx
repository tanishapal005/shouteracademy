...
import { FaGoogle, FaFacebook, FaChartBar, FaWordpress, FaMailchimp } from "react-icons/fa";
import { SiGoogletagmanager, SiSemrush, SiHubspot, SiZoho } from "react-icons/si";

// List of tools with icons
const tools = [
  { name: "Google Ads", icon: <FaGoogle size={30} className="text-red" /> },
  { name: "Facebook Ads", icon: <FaFacebook size={30} className="text-blue" /> },
  { name: "SEMrush", icon: <SiSemrush size={30} className="text-orange" /> },
  { name: "Moz", icon: <FaChartBar size={30} className="text-indigo" /> },
  { name: "Google Analytics", icon: <FaGoogle size={30} className="text-yellow" /> },
  { name: "Canva", icon: <FaWordpress size={30} className="text-purple" /> },
  { name: "Mailchimp", icon: <FaMailchimp size={30} className="text-yellow" /> },
  ...
  return (
    <div className="bg-gray-1 py-10 mt-11 mb-10 px-4 md:px-6 lg:px-10">
      ...
      <Swiper
        ...
        className="w-full"
      >
        {Array.from({ length: Math.ceil(tools.length / 2) }, (_, rowIndex) => (
          <SwiperSlide key={rowIndex}>
            <div className="grid grid-cols-1 gap-y-6 mb-14 items-center justify-center">
              {tools.slice(rowIndex * 2, rowIndex * 2 + 2).map((tool, index) => (
                <div
                  key={index}
