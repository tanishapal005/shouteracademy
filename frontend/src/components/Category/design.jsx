  ...
  const allCourses = {
    "Digital Marketing": [
      ...
      },
      {
        ...
        originalPrice: "₹3,999",
        img: category1,
        link: "/videoeditingprogram",
      },
    ],
  };

  return (
    <div className="p-4 sm:p-6 md:p-8 bg-white border-2 border-b-4 border-black rounded-lg">
      {/* Section Title */}
          ...
          <div key={course.id} className="bg-white rounded-xl shadow-lg border border-black overflow-hidden transform transition duration-300 hover:scale-105 w-full max-w-xs mx-auto">
            ...
            <div className="p-4">
              ...
              <div className="mt-4 flex flex-col gap-2">
                <button className="bg-red text-white px-4 py-2 text-xs md:text-sm rounded-md font-semibold w-full  cursor-pointer border-0 border-b-4 border-black">View Curriculum</button>
                <Link to={course.link}><button className="border-2 border-b-2 border-black px-4 py-2 text-xs md:text-sm rounded-md  cursor-pointer font-semibold w-full">Learn More</button></Link>
              </div>
            </div>
          </div>
        ))}
